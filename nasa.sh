...
scriptCore() {
    ...
    for file in *.jpg; do # rename all images to constructed name
    ...
    for file in *.png; do # rename all images to constructed name
...
if [ "$1" == "-d" -a "$3" == "-n" ]; then # if user wants detailed info along with renaming the output image
    ...
    echo
    for ((diff = $cnt1; diff >= $cnt2; diff--)); do # calls the script core funtion past those days from today that are betwenn start and end dates
